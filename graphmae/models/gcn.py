...
class GCN(nn.Module):
    ...
    def forward(self, graph, feat,  eweight=None, return_hidden=False, concat=False, datas=None, link_prediction=False):
        g = graph
        inputs = feat
        h = inputs
        hidden_list = []
        ...
        if self.norms is not None and len(self.norms) == self.num_layers:
            h = self.norms[-1](h)
        if concat:
            return self.head(torch.concat((h, datas), dim=1))
        if link_prediction:
            z = self.link_head(h)
            E = g.edges()
            neg_edge_index = dgl.sampling.global_uniform_negative_sampling(g,504378)
